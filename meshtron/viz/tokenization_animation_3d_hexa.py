...
def build_figure_hexa(mesh, steps, slots, total_blocks: int,
    ...
    faces_per_hex, verts_per_face = 6, 4
    tpv = (steps[0]["n_tok"] // len(steps[0]["emit"])) if steps else 4
    face_block = faces_per_hex * verts_per_face * tpv
    n_rows_total = len({st["row"] for st in steps})
    acc = [0] * (n_steps + 1)
    _run = 0
    for _i, _st in enumerate(steps):
        _run += _st["n_tok"] + (1 if _st["row_end"] else 0)
        acc[_i + 1] = _run

    def info_panel(blk, row, faces, mine, note=""):
        s = (f"Block: {blk}/{total_blocks}<br>Row: {row}/{n_rows_total}"
             f"<br>Encodings in Tokens<br>Faces: {faces}<br>Block Rows: {mine}")
        if faces > 0:
            s += f" (\u2212{round(100 * (1 - mine / faces))}%)"
        return s + (f"<br>{note}" if note else "")
    ...
    for tlen in range(max_step):
        ...
        if fidx >= n_steps:
            faces = (n_steps * verts_per_face * tpv if face
                     else n_steps * face_block)
            info = info_panel(total_blocks, n_rows_total, faces, acc[n_steps])
        elif a is not None:
            faces = fidx * verts_per_face * tpv if face else fidx * face_block
            blk_done = (fidx + faces_per_hex - 1) // faces_per_hex if face else fidx
            info = info_panel(blk_done, a["row"] + 1, faces, acc[fidx])
        else:
            info = info_panel(0, 0, 0, 0, note="press \u2192 to tokenize")
    ...
    if show_bar:
        ...
        for s in slots:
            ...
            shapes.append(go.layout.Shape(type="rect", xref="x", yref="y",
                                          ...
                                          y0=cy - 0.32, y1=cy + 0.32,
                                          line=dict(width=1.4,
                                                    color="#0088b0" if is_block else "#9a9490"),
                                          fillcolor=("rgba(207,230,239,0.75)" if is_block
                                                     else "rgba(234,233,233,0.9)")))
            anno = lab
            bar_annos.append(go.layout.Annotation(xref="x", yref="y", x=s["x"], y=cy + 0.40,
    ...
    labels_trace = go.Scatter(x=[s["x"] for s in slots],
                              y=[s["y"] + 0.40 for s in slots],
                              text=slot_text, mode="text",
                              textfont=dict(size=10, color=INK),
                              hoverinfo="skip").to_plotly_json()
    head = static_json + ([labels_trace] if show_bar else [])
    frames_json = [
        {"data": head + [tr.to_plotly_json() for tr in fr.data],
    ...
    if show_bar:
        ...
        # Row 1 belongs at the TOP: the axis is reversed, so lane r grows downwards.
        y_win = min(n_rows - 1 + 0.6, 8.9)     # vertikaler Fensterausschnitt
        fig.update_yaxes(visible=False, range=[y_win, -0.7], autorange=False,
                         row=1, col=2)
        # frame the token panel so it reads as its own window
        fig.layout.shapes = tuple(fig.layout.shapes or ()) + (
            go.layout.Shape(type="rect", xref="paper", yref="paper",
                            x0=0.505, x1=1.0, y0=0.0, y1=1.0,
                            line=dict(width=1.2, color="#9a9490"),
                            fillcolor="rgba(0,0,0,0)", layer="below"),)
    else:
        fig.layout.annotations = []
    fig.update_layout(
        height=900 if show_bar else 630,
        ...
        scene=dict(aspectmode="data",
                   # eye rotated 180 deg about z -> front view; magnitude = default zoom
                   camera=dict(eye=dict(x=-0.5, y=-0.5, z=0.5),
                                center=dict(x=0.32, y=-0.02, z=0.0)),
...
def generate_hexa(out_dir, path, idx, start_rule='min_theta',
    ...
    fig, max_idx, frames_json = build_figure_hexa(
        ...
        granularity=granularity, eoe=eoe, show_bar=show_bar)
    ...
    render_html(fig, out, max_idx, frames_json,
                info_div=False if show_bar else 'left')
